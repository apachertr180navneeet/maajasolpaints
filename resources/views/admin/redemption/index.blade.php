                                    ...
                                    @foreach ($redeemRequests as $request)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div class="user-info">
                                                    ...
                                                    <strong>
                                                        ...
                                                        @if (optional($request->user)->deleted_at)

                                                            <span

                                                                class="text-danger">{{ optional($request->user)->name ?? 'Guest User' }}</span>
                                            ...
                                            <td>
                                                ...
                                                @elseif ($request->transaction_type === 'product')

                                                    <div>

                                                        <strong>Product:</strong>

                                                        {{ optional($request->product)->name ?? 'No Product' }}<br>

                                                        <a href="{{ asset('images/products/' . basename(optional($request->product)->image ?? 'default.jpg')) }}"

                                                            target="_blank">

                                                            <img src="{{ asset('images/products/' . basename(optional($request->product)->image ?? 'default.jpg')) }}"
                                            ...
                                            <td>
                                                ...
                                                @if ($request->status === 'approved' && $request->approved_at)
                                                    ...
                                                    <div
                                                        ...
                                                        style="color: #28a745; font-size: 14px; font-style: italic; margin-top: 4px;">

                                                        Approved: {{ date('d/m/Y H:i', strtotime($request->approved_at)) }}
                                                ...
                                                @if (!optional($request->user)->deleted_at)
                                                    ...
                                                    @if ($request->status === 'pending')
                                                        ...
                                                        @if (in_array($request->transaction_type, ['upi', 'account']))
                                                            ...
                                                            <form action="{{ route('admin.redemption.approve', $request->id) }}" method="POST" style="display:inline;">

                                                                @csrf

                                                                <div class="form-group">

                                                                    <input type="text" name="transaction_id" class="form-control form-control-sm" placeholder="Transaction ID" value="{{ $request->transaction_id ?? '' }}" required>
                                                        ...
                                                        @elseif ($request->transaction_type === 'gift' || $request->transaction_type === 'product' || $request->transaction_type === 'cash')
                                                            ...
                                                            <form action="{{ route('admin.redemption.approve', $request->id) }}" method="POST" style="display:inline;">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <textarea name="remark" class="form-control form-control-sm" placeholder="Add Remark" rows="2">{{ $request->remark ?? '' }}</textarea>
                                                        ...
                                                        <form action="{{ route('admin.redemption.reject', $request->id) }}" method="POST" style="display:inline;">
